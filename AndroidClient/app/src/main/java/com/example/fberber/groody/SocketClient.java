...
public class SocketClient implements Runnable, Callback  {
	...
	public void sendData(byte[] data){
		if (mSocket == null){
			System.out.println("upssssssss -----");
			return;
		}
		OutputStream os = null;
		try{
			System.out.println("processs4 -----");
			os = mSocket.getOutputStream();
			os.write(data);
			os.flush();
			System.out.println("processs5 -----");
		}catch(IOException ioe){
			Log.e(LOG_TAG, ioe.toString());
		}catch(Exception e){
			Log.e(LOG_TAG, e.toString());
	...
	public void myreadData() throws IOException, ClassNotFoundException {
		if (mSocket == null) {
			System.err.println("-----  uppssss of read -----");
			return;
		}
	...
	public void temper(){
		...
		try {
			...
			System.out.println("-----------------------manuel1---"+mHost+"----"+mPort);
			Log.e(LOG_TAG, uhe.toString());
		} catch (IOException ioe) {
			System.out.println("-----------------------manuel2---"+mHost+"----"+mPort);
			Log.e(LOG_TAG, ioe.toString());
		} catch (Exception e) {
			System.out.println("-----------------------manuel3---"+mHost+"----"+mPort);
			Log.e(LOG_TAG, e.toString());
		}
	}
	...
	public void run(){
		...
		try{
			...
			System.out.println("-----------------------manuel02---"+mHost+"----"+mPort);
		} catch (UnknownHostException uhe) {
			System.out.println("-----------------------manuel1---"+mHost+"----"+mPort);
			Log.e(LOG_TAG, uhe.toString());
		} catch (IOException ioe) {
